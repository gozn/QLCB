...
import com.n18dcat093.test_database.Hoadon.THONGTINCHAMBAI;
...
//        db.execSQL(tableGV);
        db.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " ("+COLUMN_MAGV+" TEXT PRIMARY KEY, "+COLUMN_HOTENGV+" TEXT,"+COLUMN_SODT+" TEXT,"+COLUMN_HINHANH+" BLOB)");
        db.execSQL(tableMH);
        db.execSQL(tablePCB);
...
//    public GiaoVien getNameGVByID(String id) {
//        SQLiteDatabase db = this.getReadableDatabase();
//
//        Cursor cursor = db.query(sql, new String[] { KEY_ID,
...
//        return contact;
//    }
    //String sql = "SELECT MAGV, COUNT(MAGV) FROM PHIEUCHAMBAI GROUPBY MAGV";

    public String getEmployeeName(String empNo) {
        String z = "";
        String query = "SELECT HOTENGV FROM PHIEUCHAMBAI WHERE MAGV = \"%" + empNo + "%\"";
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursorC = db.rawQuery(query, null);
        while (cursorC.moveToNext()) {
            z = cursorC.getString(0);
        }
        return z;
    }
